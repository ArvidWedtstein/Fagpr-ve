CREATE OR ALTER VIEW dbo.aviw_ArvidWedtstein_MyShoppingLists
AS
...
SELECT 
  ...
  (SELECT COUNT(*) FROM dbo.atbl_ArvidWedtstein_ShoppingListsItems AS SLI WHERE SLI.ShoppingList_ID = SL.ID) AS ShoppingListItems,
  (SELECT COUNT(*) FROM dbo.atbl_ArvidWedtstein_ShoppingListsItems AS SLI WHERE SLI.ShoppingList_ID = SL.ID AND SLI.Checked = 1) AS ShoppingListItemsChecked,
  ISNULL(
      CAST(
        FLOOR(
          (SELECT COUNT(*) 
            FROM dbo.atbl_ArvidWedtstein_ShoppingListsItems AS SLI 
  ...
  CAST(
    STUFF(
      (SELECT 
        ...
        SP.[Name]
      FROM dbo.atbl_ArvidWedtstein_ShoppingListsSharedWith AS SLSW
  ...
  CAST((ISNULL(SL.[Name], '')+ISNULL((SELECT TOP 1 SP.[Name] FROM dbo.stbl_System_Persons AS SP WHERE SP.ID = SL.CreatedBy_ID), '')+STUFF(
    (
      SELECT ',' + SP.[Name]
      FROM dbo.atbl_ArvidWedtstein_ShoppingListsSharedWith AS SW
          INNER JOIN dbo.stbl_System_Persons AS SP
            ON SP.ID = SW.SharedUser_ID
      WHERE SW.ShoppingList_ID = SL.ID
    FOR XML PATH('')
  ), 1, 1, '')) AS NVARCHAR(500)) AS SearchColumn
FROM dbo.atbv_ArvidWedtstein_ShoppingLists AS SL
WHERE SL.CreatedBy_ID = dbo.sfnc_system_GetMyPersonID()
    OR EXISTS (
        SELECT 1
        FROM dbo.atbv_ArvidWedtstein_ShoppingListsSharedWith AS SW
        WHERE SW.ShoppingList_ID = SL.ID
